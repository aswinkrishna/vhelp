...
class M_quotation extends CI_Model
{
    ...
    function getJobSummary($job_request_id,$user_id, $language)
    {
        $this->db->select('*, job_request.vendor_id as vendor_id');
        $this->db->where('job_request_id',$job_request_id);
        $this->db->join('service_type','service_type.service_type_id = job_request.service_type_id and service_type_language_code ='.$language,'left');
        $this->db->join('user_table','user_table.user_id = job_request.user_id','left');
        return $this->db->get('job_request')->row();
        // $sql = " select * from job_request r, service_type s,user_table u where r.service_type_id=s.service_type_id and  service_type_language_code=" . $language . " and   job_request_id=" . $job_request_id . " and r.user_id=".$user_id;
        // $rs = $this->db->query($sql);
        // return   $row = $rs->row();
    }
...
service_type_language_code=" . $language . "  
 ...
 ";
        ...
        if ($data['status'] != "" && $data['status'] == "pending") {
            ...
            $sql .=      " and r.job_request_status = 0 ";
        }
        if ($data['status'] != "" && $data['status'] == "confirm") {

            $sql .=      " and r.job_request_status = 1 ";
        }
        if ($data['status'] != "" && $data['status'] == "rejected") {

            $sql .=      " and is_approoved= -1 ";
        }
        if ($data['status'] != "" && $data['status'] == "staff_assigned") {

            $sql .=      " and r.job_request_status= 3 ";
        }
        if ($data['status'] != "" && $data['status'] == "on_going") {

            $sql .=      " and r.job_request_status= 4 ";
        }
        ...
        if ($data['status'] != "" && $data['status'] == "cancelled") {
            ...
            $sql .=      " and r.job_request_status=10 ";
        }
...
";
    ...
    public function getAreaList($city_id,$language=1)
    {
    ...
    public function getStaffJobDetails($job_request_id){
    ...
    public function updateTempDocTable($data,$device_cart_id){
        
        $this->db->where('device_cart_id',$device_cart_id);
        $this->db->update('user_requrest_doc_temp',$data);
    ...
    public function getParentServices($service_type_id){
        $this->db->select('*');
    ...
    public function getJobDetailsById($job_request_id){
    ...
    public function getUserAddressById($adresses_id,$language =1){
        $this->db->select('*');
        $this->db->where('user_adresses_id',$adresses_id);
        $this->db->join('country','country.country_id=user_adresses.user_adresses_country and country_language_code='.$language,'left');
        $this->db->join('city','city.city_id = user_adresses.user_adresses_city and city_language_code = '.$language,'left');
        return $this->db->get('user_adresses')->row_array();
    }
    ...
    public function getServiceTypeImage($service_type_id){
    ...
    public function deleteTempOrder($job_request_temp_id){
        $this->db->where('job_request_temp_id',$job_request_temp_id);
        $this->db->delete('job_request_temp');
