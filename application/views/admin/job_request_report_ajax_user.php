...
<!--                     <th>Request type</th> -->
                    <th>Customer Response</th>
                     <th>Details</th>
<!--                    <th><?php echo $result[0]->job_request_type==2?"Provider response":"Marked price"?></th>-->
...
$postedDate = date("Y-m-d H:i:s",strtotime($rows->job_request_id));    
  ...
  ?>
                    ...
                    {
                        $user_response_status =  "Approved";
                    }
                    else if($response->user_response_status==2)
                    {
                        $user_response_status =  "Rejected";
                    }
                    ?>
...
<!--                      <td>  
                          <?php
                          if($rows->job_request_type==2 && $posts['provider_id']>0)
                          {
                          ?>
                          <select class="changeStatus" for="<?php echo $this->common_functions->encryptId($rows->job_request_id )?>" data-id="<?php echo $posts['provider_id'];?>">
                            <option value="0" <?php  echo $response->assign_status==0?"selected":"" ?>>Pending</option>
                             <option value="1" <?php  echo $response->assign_status==1?"selected":"" ?>>Confirmed</option>
                              <option value="2" <?php  echo $response->assign_status==2?"selected":"" ?>>Rejected</option>
                        </select>
                      <?php
                          }
                          else if($response->provider_amount<=0  && $rows->job_request_type==1)
                          {
                              ?>
