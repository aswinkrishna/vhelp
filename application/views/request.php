 ...
 <div class="col-md-12 auto">
     ...
     <div class="container-fluid">
         ...
         <div class="row row_margin_top">
            ...
            <div class="choose_options col-md-12 col-sm-12 select_services" id="requestModePanel">
                <aside class="steps ">
                    <div >
                        <h4 class="service-title pl-0"> Choose Service</h4>
               ...
               <div class="row">
                                    <div class="form-group col-md-6">
                                      ...
                                      <div class="input-group date" id="datepicker">
                                        <input class="form-control" name="service_date_picker" id="service_date_picker" readonly placeholder="YYYY-MM-DD" value="<?=date('Y-m-d')?>" /><span style="padding: 0" class="input-group-append input-group-addon"><span class="input-group-text"><i class="fa fa-calendar"></i></span></span>
                                      </div>
                                    </div>
                                    <div class="form-group col-md-6" style="display:none;">
                                      <label>Time</label>
                                      <div class="input-group time" id="timepicker">
                                        <input class="form-control" name="service_time_picker" id="service_time_picker" placeholder="HH:MM AM/PM"/><span class="input-group-append input-group-addon"><span class="input-group-text"><i class="fa fa-clock-o"></i></span></span>
                                      </div>
                                    </div>
  ...
  setTimeout(function(){ $(inputElement).css("display","none") }, 3000);
    ...
    $(document).delegate("#fileUploadNext","click",function(e){
       ...
       $(document).delegate("#btnLocationNext","click",function(e)
     {
           ...
           var valdate = $("#service_date_picker").val();
           var valTime  =  $(".badge-pill.active").html();
           if(valdate=="")
           {
               
                 swal("Please select a valid Date and time");
                 $("#service_date_picker").css("border-color","red");
                 error=1;
                 return false;
           }
           else
           {
                $("#service_date_picker").css("border-color","");
           }
    ...
    // alert(window.user_type);
      ...
      $('#btnBookNow').attr('disabled',true);
            ...
            //   var state                 =  $("#us5-state").val();             
               ...
               var service_date     =  $('#service_date_picker').val();
               var serviceTime      =  $(".badge-pill.active").html();
...
//selectedSummary+="</ul>";
    ...
    function loadSummaryUI(priceArr){
      ...
      var html = '';
      var service_mode  = $("input[name=service_request_type]:checked").val();
      
      var service_mode_label    = "";
      
      if(service_mode == 2)
        service_mode_label  = "Weekly Service";

      else if(service_mode == 3)
        service_mode_label  = "Monthly Service";

      else if(service_mode == 1)
        service_mode_label  = "One Time Service";

    if(service_mode_label != "")
    {
      html = html + '<li id="lidynamicQues2140">Service Mode ?</li>';
      html = html + '<li id="lidynamicQues2140">'+service_mode_label+'</li>';
    }
...
<script>
   ...
   // });
    ...
    function updateSlotTime(){
        ...
        var todayDate = new Date();
        var selectedDate= $('#service_date_picker').val();
        // alert(selectedDate);
        selectedDate = new Date(selectedDate);
    ...
    $(document).delegate("#requestModeNext","click",function(e){
        ...
        if($("input[name=service_request_type]:checked").length == 0)
        {
            error=1;
            swal("Please select any one of these !");
        }
...
$(document).ready(function(){
    ...
    $("#service_date_picker").change(function(){
        updateSlotTime();
    });
    ...
    $(".week_days").click(function(){
        changeDateWithWeekSelect();
    });
    function changeDateWithWeekSelect(){
        ...
        if(now.getHours() > 12 && $(".week_days:checked").length > 0)
        {
            const d = new Date();
            for(i=0;i<=6;i++)
            {
                ...
                var n = d.getDay();
                if(inArray(n,weekdays))
                {
                    // alert(d);
                    $("#service_date_picker").val(moment(d).format('YYYY-MM-DD'));
                    break;
                }
            }
        }
        ...
        {
            $("#service_date_picker").val(dateString);
        }
        $("#service_date_picker").trigger("change");
    }
